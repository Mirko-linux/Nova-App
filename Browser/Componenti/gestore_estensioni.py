...
class GestoreEstensioni(QDialog):
    ...
    # Questa funzione non ha senso in un gestore estensioni standalone
    ...
    #     try:
    #         # Codice per importare ed eseguire il modulo dell'estensione
    ...
    #         sys.path.insert(0, os.path.dirname(script_path)) # Aggiungi la directory dello script al PYTHONPATH
    #         spec.loader.exec_module(module)
    #         sys.path.pop(0) # Rimuovi la directory per evitare conflitti

    #         if hasattr(module, 'activate'):
    #             # Qui dovresti passare un riferimento al browser se l'estensione ne ha bisogno
    #             # Dato che questo è uno script standalone, non c'è un browser da passare.
    #             # Questo è il punto critico: l'estensione potrebbe non funzionare pienamente
    #             # senza il contesto del browser.
    #             QMessageBox.information(self, "Estensione Eseguita",
    #                                     f"Estensione '{nome_estensione}' attivata con successo (solo logica interna).")
    #             # module.activate(self.browser_instance) # Se ci fosse un browser istanziato qui
    #         else:
    #             QMessageBox.information(self, "Estensione Eseguita",
    #                                     f"Script '{script_file}' dell'estensione '{nome_estensione}' eseguito (nessuna funzione 'activate' trovata o necessaria).")
