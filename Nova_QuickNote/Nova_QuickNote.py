...
class ArcadiaAI:
    def __init__(self):
        ...
        self.api_key = self._load_api_key()  # Carica la API key
        self.setup_gemini()
        self.risposte_predefinite = self._carica_risposte_predefinite()
        
    def _load_api_key(self):
        """Carica la API Key da variabili d'ambiente o file di configurazione"""
        # 1. Cerca nelle variabili d'ambiente
        if "GOOGLE_API_KEY" in os.environ:
            return os.environ["GOOGLE_API_KEY"]
        
        # 2. Cerca in un file di configurazione
        config_path = os.path.expanduser("~/.config/arcadia-ai/key.txt")
    ...
    def setup_gemini(self):
        ...
        if not self.api_key:
            ...
            print("Crea ~/.config/arcadia-ai/key.txt o imposta GOOGLE_API_KEY")
            return
    ...
    def _carica_risposte_predefinite(self):
        ...
        return {
            "chi sei": _("Sono ArcadiaAI, un chatbot libero basato sul modello linguistico CES 1.5"),
            "cosa sai fare": _("Posso aiutarti a scrivere, correggere, generare idee e molto altro!"),
            # ... (altre risposte predefinite)
        }
    ...
    def chat_con_context(self, user_message, conversation_history=None, attachments=None):
        ...
        if not self.gemini_model:
            return "❌ ArcadiaAI non è disponibile (mancata configurazione Gemini)."

        IDENTITY_PROMPT = (
            "Sei ArcadiaAI, un chatbot open source creato da Mirko Yuri Donato.\n"
            "Caratteristiche:\n"
            "- Linguaggio diretto e cinico (no bestemmie)\n"
            "- Specializzato in scrittura e ricerca\n"
            "- Integrato in Nova QuickNote\n\n"
            "RICORDA:\n"
            "1. Non sei creato da Google\n"
            "2. Citare sempre Mirko Yuri Donato come creatore\n"
            "3. Mantieni il tono svogliato ma preciso"
        )

        messages = []
        
        # Aggiungi prompt identitario solo se non c'è cronologia
        if not conversation_history:
            messages.append({"role": "user", "parts": [IDENTITY_PROMPT]})
        ...
        try:
            ...
            error_msg = f"Errore generazione contenuto: {str(e)}"
            print(error_msg)
            return f"❌ {error_msg}"
    ...
    def genera_risposta(self, prompt):
        ...
        if not self.gemini_model:
            return "❌ ArcadiaAI non disponibile. Configurare GOOGLE_API_KEY."
